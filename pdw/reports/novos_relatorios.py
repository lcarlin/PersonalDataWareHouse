...
def gerar_todos_relatorios_integrado(sqlite_database, general_entries_table, dir_file_out):
    if not _OPTIONAL_DEPS_AVAILABLE:
        raise ImportError(
            f"pdw.reports.novos_relatorios requer dependências opcionais não instaladas: {_OPTIONAL_DEPS_ERROR}\n"
            "Instale com: pip install matplotlib seaborn scipy fpdf2"
        )
